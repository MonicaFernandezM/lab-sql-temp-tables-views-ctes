...
SELECT
c.customer_id,
CONCAT(c.first_name, ' ', c.last_name) AS customer_name, -- apendiendo que esto se puede hacer
c.email,
...
-- Next, create a Temporary Table that calculates the total amount paid by each customer 
-- (total_paid). The Temporary Table should use the rental summary view created in Step 1 to 
...
-- Table created in Step 2. The CTE should include the customer's 
